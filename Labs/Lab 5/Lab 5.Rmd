...
untidy2.school = untidy.school %>%
                    unite(col = 'CID', c(school, student), sep = ".", remove = FALSE)
...
untidy3.school = untidy2.school %>%
                    ...
                    gather(key = 'Year', value = value, `2017`:`2018`)
...
final.tidy.school = untidy.school %>%
                      rename("2017" = value2017, "2018" = value2018) %>%
                      gather(key = "Year", value = "Value", `2017`:`2018`, convert = TRUE) %>%
                      spread("type", "Value") %>%
                      complete(school, student, Year) %>% 
                      unite(col = 'CID', c(school, student), sep = ".", remove = FALSE) 
...
If I were the district superintendent, I would distribute the King's ransom to the schools which showed the most improvement. For example, the last school all the way on the right did not show that much improvement because it already had a pretty high average, wherease the second orange school showed a considerable improvement. We can come up with some metric to measue this improvement which could be as simple as subtracting the average scores from 2018 with the average scores from 2017, and then dividing up the money based on this metric. 
...
Looking at the first graph, we can see a clear linear positive improvement in the scores of the student between the two years, which would incline one to believe that the students improved in their math knowledge. However the second graph which shows percentiles has a lot of noise and does not give any clear indication of what is happening. If we are to believe that the students did genuinely improve their math knowledge, then we would see a linear relationship in the percentiles as well, where people who were performing well in the class continue to perform well, while the students who used to perform worse, now perform slightly better. But this graph is too noisy to make any accurate conclusions. 

Also another question I have, which requires more research, is whether there was any foul play by the schools, wherein they might have focused more on the questions which are likely to appear on the exams, and basically training the students on those questions, which wouldn't actually improve their math knowledge as a whole. But again, this is just a hypothesis which would require data and an in-depth analysis to answer. 
