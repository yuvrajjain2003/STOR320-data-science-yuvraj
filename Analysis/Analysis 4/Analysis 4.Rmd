...
while (i <= 8) {
  ...
  while (j <= 8) {
    ...
    OUT.MAE[i, j] = MAE.func(actual = DATA3$W, predict = DATA3$.fitted)
    j = j + 1
  }
  i = i + 1
}
...
BEST5.DATA2 = BEST5.DATA %>%
  gather(key = "Model", value = "Predict", c(First, Second, Third, Fourth, Fifth), factor_key = TRUE)
...
```{r,eval=T}
ggplot(BEST5.DATA2 %>% filter(L == 920)) +
  geom_point(mapping = aes(x = D, y = W), alpha = 0.05, stroke = 0) +
  geom_line(mapping = aes(x = D, y = Predict, color = Model)) +
  xlab("Julian Day") + 
  ylab("Max Water Temperature") +
  theme_minimal()
```

Answer *(2 Points)*:  I would say that all five models are very similar to each other, since they more or less overlap each other for their predicted values when applied to location 920. 
...
Answer: Looking at the graphics above, I would pick $I = 3$ and $J = 4$. $I = 3$ already gives a pretty low avgMAE value as we see from the graph, and as for $J$, I believe that although $J = 7$ might have a lower avg MAE, the difference between the avgMAE values for $J = 4$ and $J = 7$ is not actually taht significant. Therefore, being biased towards the simpler model, I would pick $I = 3$ and $J = 4$ since I believe it has a better chance of predicting on out-of-sample data.

Overall, selecting $I=3$ and $J=4$ would give a good balance between model complexity and performance on out-of-sample data.
...
BEST.func=function(data){
  output = as.numeric(data[which.min(data$MAE), 1:2])
  return (output)
}
...
Code and Output *(3 Points)*:
```{r,eval=F}
DATA.LOC920 <- DATA %>% filter(L == 920)
ggplot(DATA.LOC920) +
  geom_point( mapping = aes(x = W, y = predict(train.model.func(DATA.LOC920,2,6))) ) +
...
Code and Output *(3 Points)*:
```{r,eval=F}
DATA.LOC328 <- DATA %>% filter(L == 328)
ggplot(DATA.LOC328) +
  geom_point(aes(D, residuals(train.model.func(DATA.LOC328,2,6))))+
