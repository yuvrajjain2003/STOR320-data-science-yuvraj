...
You want to find the smallest model that accurately predicts. If a linear model has a RMSE which is only slightly worse than a multiple polynomial model, then it's better to just use a linear model. You always want to choose the smallest model that accurately predicts. Simpler models are more robust because they're less affected by extraneous variables. For example, water temperature might be affected by other things other than air temperature and day of the year for example humidity etc. 

**Always be biased towards the simpler model.**